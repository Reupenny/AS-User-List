...
function add_to_favorites_button_shortcode()
{
    ...
    if ($icon_style == 'default') {
        ...
        if ($favorite) {
            return '<button data-post-id="' . $post->ID . '" class="remove-favorite button wc-forward wp-element-button">' . $icon_remove_text . '</button>' . $modal;
        } else {
            return '<button id="add-to-favorites" class="add-to-favorites button wc-forward wp-element-button" data-post-id="' . $post->ID . '">' . $icon_add_text . '</button>' . $modal;
        }
    }
    if ($icon_style == 'bookmark') {
        ...
        $favorite = $wpdb->get_row("SELECT * FROM $table_name WHERE user_id = $current_user->ID AND post_id = $post->ID");
        if ($favorite) {
            return '<button data-post-id="' . $post->ID . '" class="remove-favorite bettter_fav_button"><span class="icon icon-bookmark"></span> ' . $icon_remove_text . '</button>' . $modal;
        } else {
            return '<button id="add-to-favorites" class="add-to-favorites bettter_fav_button" data-post-id="' . $post->ID . '"><span class="icon icon-bookmark-outline"></span> ' . $icon_add_text . '</button>' . $modal;
        }
    }
    if ($icon_style == 'heart') {
        global $wpdb, $post;
        ...
        $favorite = $wpdb->get_row("SELECT * FROM $table_name WHERE user_id = $current_user->ID AND post_id = $post->ID");
        if ($favorite) {
            return '<button data-post-id="' . $post->ID . '" class="remove-favorite bettter_fav_button"><span class="icon icon-heart"></span>  ' . $icon_remove_text . '</button>' . $modal;
        } else {
            return '<button id="add-to-favorites" class="add-to-favorites bettter_fav_button" data-post-id="' . $post->ID . '"><span class="icon icon-heart-outline"></span> ' . $icon_add_text . '</button>' . $modal;
        }
    }
}
