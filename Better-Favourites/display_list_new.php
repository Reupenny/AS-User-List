...
function favorites_list_shortcode()
{
    ...
    if (count($favorites) > 0) {
        $output = '<div class="better_favourites_list">';
        if ($Better_fav_list_title) {
            $output .= '<h3>' . esc_html($Better_fav_list_title) . '</h3>';
        }
        ...
        foreach ($favorites as $favorite) {
            ...
            if ('product' === $post->post_type) {
                ...
                $output .= '<tr>';
                $output .= '<td class="Better_fav_mobile">';
                $output .= '<a class="Better_fav_mobile_img" href="' . esc_url(get_permalink($post->ID)) . '">';
                $output .= get_the_post_thumbnail($post->ID, 'thumbnail');
                $output .= '<br>' . esc_html($post->post_title) . '</a></td>';
                $output .= '<td class="Better_fav_mobile_hide">';
                ...
                $output .= '<td class="Better_fav_mobile_hide">' . esc_html($stock_status) . '</td>';
                $output .= '<td class="Better_fav_mobile_hide">';
                $output .= '<a href="' . esc_url(get_permalink($post->ID)) . '" class="Better_fav_view_product_button">View Product</a></td>';
                $output .= '<td class="Better_fav_mobile_hide">';
                $output .= '<div data-post-id="' . $post->ID . '" class="remove-favorite bettter_fav_button bettter_fav_cross"><i class="icon icon-close-outline" alt="Remove from list"></i></div></td>';
                $output .= '</tr>';
            }
        }
        $output .= '</table>';
...
//new Better_Favorites_Shortcode();
